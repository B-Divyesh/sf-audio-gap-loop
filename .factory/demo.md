...
Demo records use the IndexedDB database `demo:audio-gap-loop` and `demo:`-prefixed localStorage keys. The normal player uses `audio-gap-loop` and unprefixed keys. **Reset demo** clears and reseeds only the demo namespace. **Start for real** clears all demo keys and deletes the demo database before opening `/`.
